...
export default function PageButtonsForm({ user, page }) {
  // Check if 'page' is defined before accessing its 'buttons' property
  ...
  return (
    <SectionBox>
      <form action={saveButtons}>
        <h2 className="text-2xl font-bold mb-4 rounded">Buttons</h2>
        <ReactSortable
          handle=".handle"
          list={activeButtons}
        ...
        <div className="flex flex-wrap gap-2 mt-4 border-y py-4 rounded">
          {availableButtons.map((b) => (
            <button
              key={b.key}
              type="button"
              onClick={() => addButtonToProfile(b)}
              className="flex items-center gap-1 p-2 rounded bg-gray-200"
            >
              <FontAwesomeIcon icon={b.icon} />
        ...
        <div className="max-w-xs mx-auto mt-8 rounded">
